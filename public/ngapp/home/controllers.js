...
controllers.controller('DashboardBodyCtrl', ['$scope', '$stateParams', '$modal',
  ...
  $scope.onDrop = function ($event, $data, array, index) {
    var alert = {
      type: 'danger',
      message: "Error dropping dependency.",
    ...
    if (deps.length > 0) {
      $scope.addAlert({
        type: 'danger',
        message: "Dependency '" + $data.Name + "' aleardy registered.",
        icon: 'glyphicon glyphicon-remove'
      });
      return;
    }
  ...
  $scope.handleDependency = function (env, dep, action) {
    ...
    if (action === 'accept') {
      dep.Status = 'OK';
      dep.Host = 'internal-router.services.example.com';
      dep.Port = 45935;
      $scope.addAlert({
        type: 'success',
        message: "Dependency '" + dep.Name + "' registered successfully.",
        icon: 'glyphicon glyphicon-ok'
      });
    } else {
      ...
      $scope.addAlert({
        type: 'success',
        message: "Dependency '" + dep.Name + "' removed successfully.",
        icon: 'glyphicon glyphicon-ok'
      });
    }
  };
...
controllers.controller('EnvContentCtrl', ['$scope', '$modal', '$stateParams', 'appsFactory',
  ...
  $scope.deleteSha = function (sha) {
    ...
    modalInstance.result.then(function (name) {
      ...
      $scope.addAlert({
        type: 'success',
        message: "Sha '" + name + "' deleted successfully.",
        icon: 'glyphicon glyphicon-ok'
      });
      $scope.isShaInfoEnabled = false;
    }, function (result) {
